...
class Scope(models.Model):
    ...
    identifier = models.CharField(
        _('Scope ID'), max_length=100,
        help_text=_('Scope identifier. Usually in form of `app_name:view_name`.'),
        unique=True)

    title = models.CharField(
        _('Scope title'), max_length=250,
        help_text=_('Scope human-friendly name.'))

    status = models.PositiveIntegerField(
        _('Status'), db_index=True, choices=STATUS_CHOICES, default=STATUS_ENABLED)
...
class Client(models.Model):
    ...
    date_registered = models.DateTimeField(
        _('Registered at'), auto_now_add=True)

    title = models.CharField(
        _('Title'), max_length=100, unique=True)

    user = models.ForeignKey(
        USER_MODEL, verbose_name=_('Registrant'),
        help_text=_('User who registered this client.'),
    ...
    link = models.URLField(
        _('URL'),
        help_text=_('Application webpage URL.'), null=True, blank=True)

    identifier = models.CharField(
        _('Identifier'), max_length=250,
        help_text=_('Public client identifier. <i>Generated automatically if empty.</i>.'),
        unique=True, blank=True)

    token_lifetime = models.IntegerField(
        _('Token lifetime'),
        help_text=_('Time in seconds after which token given to the application expires.'),
        null=True, blank=True)

    password = models.CharField(
        _('Password'), max_length=250,
        help_text=_(
            'Secret that can be used along with an identifier as username to authenticate with HTTP Basic scheme.'),
        blank=True)

    type = models.IntegerField(
        _('Type'),
        help_text=_(
            '<b>Confidential</b> &#8212; Clients capable of maintaining the confidentiality '
    ...
    scopes = models.ManyToManyField(
        ...
        help_text=_(
            'The scopes client is restricted to. <i>All registered scopes will be available '
    ...
    hash_sign_supported = models.BooleanField(
        ...
        help_text=_(
            'Should be checked if this client supports fragment component (#) in the HTTP "Location" '
    ...
    @classmethod
    def generate_indentifier(cls) -> str:
...
class RedirectionEndpoint(models.Model):
    ...
    uri = URLSchemeField(
        _('URI'),
        help_text=_(
            'URI or URI scheme for authorization server to redirect client when an interaction '
...
class AuthorizationCode(models.Model):
    ...
    user = models.ForeignKey(
        ...
        help_text=_('The user authorization is granted for.'),
        on_delete=models.CASCADE)

    client = models.ForeignKey(
        Client, verbose_name=_('Client'),
        help_text=_('The client authorization is granted for.'),
        on_delete=models.CASCADE)

    uri = URLSchemeField(
        _('Redirect URI'),
        help_text=_('The URI authorization is bound to.'))
...
class Token(models.Model):
    ...
    access_token = models.CharField(
        _('Access Token'), max_length=32,
        help_text=_('Token to be used to access resources.'),
        unique=True, blank=True)

    refresh_token = models.CharField(
        _('Refresh Token'), max_length=32,
        help_text=_('Token to be used to refresh access token.'),
    ...
    user = models.ForeignKey(
        USER_MODEL, verbose_name=_('User'),
        help_text=_('The user token is issued for.'), null=True, blank=True,
        on_delete=models.CASCADE)

    client = models.ForeignKey(
        Client, verbose_name=_('Client'),
        help_text=_('The client application token is issued for.'),
        on_delete=models.CASCADE)

    code = models.ForeignKey(
        AuthorizationCode, verbose_name=_('Code'),
        help_text=_('Authorization code used to generate this token.'),
        null=True, blank=True, on_delete=models.CASCADE)

    scopes = models.ManyToManyField(
        Scope, verbose_name=_('Scopes'),
        help_text=_('The scopes token is restricted to.'), blank=True)
